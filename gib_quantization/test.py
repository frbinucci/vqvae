...
def main():
    ...
    for nz in n_components_array:
        quantizer = GaussianIBRateAllocator(R_tot=64)
        ...
        #Ideal NMSE
        nmse_db_array[component_index] = compute_nmse(Ytest,Y_hat)
        ...
        nmse_quantization_array[component_index] = compute_nmse(Ytest,Y_hat_quantized)

        component_index+=1

    plt.plot(n_components_array,nmse_db_array,marker='v')
    plt.plot(n_components_array,nmse_quantization_array,marker='o')
    plt.grid()
    plt.show()
